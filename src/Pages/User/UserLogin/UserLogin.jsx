...
export default function UserLogin() {
  ...
  async function onSubmit(values) {
    ...
    setErrorLogin("");
    try {
      const _response = await axios.post(`${import.meta.env.VITE_BASE_URL}api/v1/user/auth/login`, { ...values });

      if (_response?.data?.status === 1) {
        sessionStorage.setItem("accessToken", _response?.data?.data?.accessToken);
        sessionStorage.setItem("user", JSON.stringify(_response?.data?.data?.user));
  ...
  return (
    <>
      <div className="container relative h-screen flex-col items-center justify-center md:grid lg:max-w-none lg:grid-cols-2 lg:px-0">
        <Link to="/user-register" className={"absolute right-4 top-4 md:right-8 md:top-8"}>
          Register
        </Link>
        ...
        <div className="lg:p-8">
          <div className="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[350px]">
            <div className="flex flex-col space-y-2">
              ...
              {successLogin !== "" && (
                <div className={`text-white rounded-md bg-green-600 px-4 py-2`}>{successLogin}</div>
              )}
            </div>
          </div>
        </div>
